...
class ShortLongReflectionMemory(ShortLongMemory):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(embedding_size, importance_weight, stm_K, ltm_K, **kwargs)
        self.reflection_threshold = reflection_threshold
        self.reflecting = False
        self.aggregate_importance = 0.0

    def _get_topics_of_reflection(self, last_k: int = 50):
        msg = Msg("user", Template.get_topics_of_reflection_prompt([_.content for _ in self.ltm_memory[-last_k:]]), role="user")
    ...
    def _get_insights_on_topic(self, topic: Msg) -> List[str]:
        ...
        msg = Msg("user", Template.get_insights_on_topic_prompt(retrieved_memories, topic, True), role="user")
        prompt = self.model.format(msg)
    ...
    def add(self, memory: Msg):
        ...
        if self.aggregate_importance >= self.reflection_threshold and not self.reflecting:
