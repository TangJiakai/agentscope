...
class SeekerAgent(AgentBase):
    ...
    def determine_search_jobs_fun(self, search_job_number: int, **kwargs):
        ...
        search_jobs = self.env.get_job_infos(search_job_ids)

        self.memory.add(mem = Msg(
    ...
    def determine_apply_job_fun(self, search_jobs: list, **kwargs):
        ...
        tht = self.reflect(current_action="Select the jobs which you want to submit your resume")
        msg = Msg("user", tht.content + Template.determine_apply_jobs_prompt(search_jobs), role="user")
        ...
        response = self.model(prompt, parse_func=parse_func)
        apply_job_ids = response.raw
        apply_jobs = [job for job in search_jobs if job.id in apply_job_ids]

        self.memory.add(mem = Msg(
            "assistant", Template.determine_apply_jobs_memory(apply_jobs), role="assistant"
        ))
    ...
    def run_fun(self, **kwargs):
        if self.seeker.status != "unemployed":
