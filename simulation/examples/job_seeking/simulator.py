...
from utils.build_index import build_dense_index
from agent import SeekerAgent, JobAgent, CompanyAgent
...
class Simulator(BaseSimulator):
    def __init__(self):
        self.config = load_yaml(os.path.join(CONFIG_DIR, SIMULATION_CONFIG))
    ...
    def _init_agentscope(self):
        agentscope.init(
            ...
            model_configs=os.path.join(CONFIG_DIR, self.config["model_configs_path"]),
            use_monitor=False,
        )

    def _init_agents(self):
        # Load configs
        memory_config = load_json(os.path.join(CONFIG_DIR, MEMORY_CONFIG))
        seeker_configs = load_json(os.path.join(CONFIG_DIR, SEEKER_AGENT_CONFIG))
        job_configs = load_json(os.path.join(CONFIG_DIR, JOB_AGENT_CONFIG))
        company_configs = load_json(os.path.join(CONFIG_DIR, COMPANY_AGENT_CONFIG))

        # Init agents
        self.seeker_agents = [SeekerAgent(**config["args"], memory_config=memory_config) for config in seeker_configs]
        self.job_agents = [JobAgent(**config["args"], memory_config=memory_config) for config in job_configs]
        self.company_agents = [CompanyAgent(**config["args"], memory_config=memory_config) for config in company_configs]
    ...
    def _make_decision_round(self, seeker_agents):
        ...
        for seeker_agent in seeker_agents:
            seeker_agent(Msg("assistant", None, fun="make_decision", params={
                "agents": self.agents,
            }))
        ...
        for seeker_agent in seeker_agents:
            if seeker_agent.decision == 0:  # No any offers, and continue to search for jobs
                ...
                seeker_agent.memory_info["final_offer"] = self.agents[seeker_agent.final_offer_id].job
                company_name = self.agents[seeker_agent.final_offer_id].job.company_name
                seeker_agent.update_job_condition(company_name, self.agents[seeker_agent.final_offer_id].job)
                logger.info(f"{seeker_agent.name} accepts the offer {self.agents[seeker_agent.final_offer_id].name}.")
            elif seeker_agent.decision == 2:    # Wait for the waitlist offer
                seeker_agent.memory_info["waiting_time"] += 1
                logger.info(f"{seeker_agent.name} rejects all offers, and waits for {[self.agents[x].name for x in seeker_agent.wl_jobs_dict]}.")
        ...
        for job_agent in job_agents:
            logger.info(f"{job_agent.name} offers {[self.agents[x].name for x in job_agent.offer_seeker_ids]}, waitlists {[self.agents[x].name for x in job_agent.wl_seeker_ids]}.")
    ...
    def _one_round(self, job_dense_index):
        ...
        for seeker_agent in self.seeker_agents:
            if seeker_agent.seeker.status == "unemployed" or seeker_agent.seeking:
                ...
                seeker_agent(Msg("assistant", None, fun="add_memory",))
        ...
        for seeker_agent in seeker_agents:
            seeker_agent.search_job_ids = random.sample(seeker_agent.job_id_pool, seeker_agent.search_job_number)
            seeker_agent.memory_info["search_jobs"] = [self.agents[x].job for x in seeker_agent.search_job_ids]
        ...
        for job_agent in job_agents:
            job_agent(Msg("assistant", None, fun="cv_screening", params={
                "apply_seekers": [self.agents[seeker_id].seeker for seeker_id in job_agent.apply_seeker_ids],
                "excess_cv_passed_n": self.config["excess_cv_passed_n"],
            }))
            job_agent.memory_info["apply_seekers"] = [self.agents[x].seeker for x in job_agent.apply_seeker_ids]
            job_agent.memory_info["cv_passed_seeker_ids"] = job_agent.cv_passed_seeker_ids
            logger.info(f"{job_agent.name} passes the cv screening for {[self.agents[x].name for x in job_agent.cv_passed_seeker_ids]} seekers.")
        ...
        for seeker_agent in seeker_agents:
            ...
            logger.info(f"{seeker_agent.name} passes the cv screening for {[self.agents[x].name for x in seeker_agent.cv_passed_job_ids]} jobs.")
        ...
        for job_agent in job_agents:
            ...
            for i, seeker_id in enumerate(job_agent.wl_seeker_ids):
                self.agents[seeker_id].wl_job_dict[job_id] = {"rank": i+1, "wl_n": len(job_agent.wl_seeker_ids)}
    ...
    def run(self):
        job_dense_index = build_dense_index(self.config, self.seeker_agents, self.job_agents)
