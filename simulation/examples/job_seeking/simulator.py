...
class Simulator(BaseSimulator):
    ...
    def _init_agents(self):
        ...
        for config in seeker_configs:
            ...
            config["args"]["embedding"] = get_embedding(cv, self.config["embedding_api"])

        for config in interviewer_configs:
            name, jd, jr = config["args"]["name"], config["args"]["jd"], config["args"]["jr"]
            config["args"]["embedding"] = get_embedding(f"{name} {jd} {' '.join(jr)}", self.config["embedding_api"])

        for config in company_configs:
            name, cd = config["args"]["name"], config["args"]["cd"]
            config["args"]["embedding"] = get_embedding(f"{name} {cd}", self.config["embedding_api"])
        ...
        seeker_agents = [
            SeekerAgent(
                env_agent=env_agent,
                **config["args"],
                to_dist=DistConf(host=config["args"]["host"], port=config["args"]["port"]),
            )
            for config in seeker_configs
        ]
        interviewer_agents = [
        ...
        index.add(np.array([config["args"]["embedding"] for config in interviewer_configs]))
        for config in seeker_configs:
            _, job_index = index.search(
                np.array([config["args"]["embedding"]]), self.config["pool_size"]
            )
            config["args"]["job_ids_pool"] = [interviewer_agents[index].agent_id for index in list(job_index[0])]
            
        results = []
        for agent, config in zip(seeker_agents, seeker_configs):
            results.append(agent(get_assistant_msg(
                fun="set_attr", 
                params={"attr": "job_ids_pool", "value": config["args"]["job_ids_pool"]}))
            )
        for res in results:
            res["content"]
