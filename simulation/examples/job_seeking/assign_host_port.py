...
def main(args):
    ...
    print("len(interview_configs):", len(interviewer_configs))

    total_agent_num = len(seeker_configs) + len(interviewer_configs)
    agent_num_per_server = math.ceil(total_agent_num / available_port_num)
    print("agent_num_per_server:", agent_num_per_server)

    for i, agent_config in enumerate(seeker_configs + interviewer_configs):
