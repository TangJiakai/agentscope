...
def model_is_embedding(
    model_name: str, trust_remote_code: bool, quantization: Optional[str]
) -> bool:
    return ModelConfig(
        model=model_name,
...
async def build_async_engine_client_from_engine_args(
    ...
    if (
        model_is_embedding(
            engine_args.model, engine_args.trust_remote_code, engine_args.quantization
        )
    ...
    # Otherwise, use the multiprocessing AsyncLLMEngine.
        ...
        logger.info("Multiprocessing frontend to use %s for RPC Path.", rpc_path)
        ...
        logger.info("Started engine process with PID %d", rpc_server_process.pid)
...
async def warmup(args, llm_engine):
    ...
    def extract_macro_instructions(content):
        macro_pattern = (
            r"\{% macro\s+[a-zA-Z_]+_instruction\(\)\s*%\}([\s\S]*?)\{% endmacro %\}"
        )
    ...
    for instruction in instructions:
        ...
        blocks = llm_engine.engine.scheduler[
            0
        ].block_manager.gpu_allocator.evictor.free_table
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt-dir", type=str, help="The file path to the prompt file", default=None
    )
