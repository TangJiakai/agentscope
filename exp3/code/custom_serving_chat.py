...
def create_logit_processor(i):
    def logit_processor(past_tokens_ids, logits_row):
        # 创建一个mask，保留前i个logits，将其余部分设置为-100
        mask = torch.full_like(logits_row, -math.inf)
        mask[:i] = logits_row[:i]
        # mask = torch.full((15 + logits_row.shape[0],), -math.inf)
        # mask[15:15+i] = logits_row[:i]
        return mask
    logit_processor.is_choice_processor = True
